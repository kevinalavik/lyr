-- https://github.com/PeterHickman/SimpleLuaGames/blob/master/blocks/blocks.lua
require('strict')
  ...
  for r=1,4 do
    ...
    for c=1,4 do
      if board[r][c] == 0 then
        ...
        io.write(string.char(string.byte('a') - 1 + board[r][c]))
      end
    end
    print(":R")
  end
...
function add_blocks_at_random(number)
...
function get_user_move()
  ...
  while waiting_for_input do
    ...
    print("Enter a direction to move: U, D, L or R")
    here = io.read()

    if here == 'U' or here == 'u' then
      here = 'u'
    elseif here == 'D' or here == 'd' then
      here = 'd'
    elseif here == 'L' or here == 'l' then
      here = 'l'
    elseif here == 'R' or here == 'r' then
      here = 'r'
    else
      waiting_for_input = true
      print("That was not a valid instruction")
    end
  end
...
while can_make_a_move() do
  ...
  if move_board(here) == 'x' then
    print("Unable to move in that directions (" .. here .. ")")
  else
    add_new_block(here)
  end
end
