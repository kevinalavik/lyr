...
#define __log(color, subsys, fmt, ...) \
	klog_printf(color, subsys, fmt, ##__VA_ARGS__)

#if _TRACE == 1
#define log_trace(subsys, fmt, ...) \
	__log(LOG_CLR_TRACE, subsys, fmt, ##__VA_ARGS__)
#else
...
#define log_debug(subsys, fmt, ...) \
	__log(LOG_CLR_DEBUG, subsys, fmt, ##__VA_ARGS__)
#else
...
#define log_info(subsys, fmt, ...) \
	__log(LOG_CLR_INFO, subsys, fmt, ##__VA_ARGS__)
#else
