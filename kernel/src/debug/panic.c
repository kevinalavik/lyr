...
{
	log_err("panic", "Page Fault:");
	log_err("panic", "  Present:  %s", (err & 1) ? "yes" : "no");
	log_err("panic", "  Write:    %s", (err & 2) ? "write" : "read");
	log_err("panic", "  User:     %s", (err & 4) ? "user" : "kernel");
	log_err("panic", "  Reserved: %s", (err & 8) ? "yes" : "no");
									  ...
									  ...)
{
	...
	if (fmt) {
		...
		log_err("panic", "reason: No message provided.");
	}
	...
	if (frame) {
		if (frame->vector < 32) {
			if (frame->vector == 14) {
				log_err("panic", "CR2: 0x%lx", frame->cr2);
