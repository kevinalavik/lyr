					  ...
					  uint8_t polarity, uint8_t trigger_mode, uint8_t lapic_id)
{
	...
	if (!found) {
		log_err("apic", "Couldn't find IOAPIC for GSI %u", gsi);
		return;
	}

	uint32_t pin = gsi - ioapics[i]->gsi_base;

	ioapic_write((uint64_t)PHYS_TO_VIRT(ioapics[i]->addr), IOAPICREDTBLL(pin),
				 redent.bytes.low);
	ioapic_write((uint64_t)PHYS_TO_VIRT(ioapics[i]->addr), IOAPICREDTBLH(pin),
				 redent.bytes.high);

	log_trace("apic",
			  "Set IOAPIC redirection entry for vector %u, gsi %u (0x%lx%lx)",
...
void apic_init()
{
	...
	apic_cpu_init(0); // only bsp for now

	for (size_t i = 0; i < ioapic_count; i++) {
		log_trace("apic", "Mapping I/O APIC #%u (0x%llx -> 0x%llx)...", i,
				  ioapics[i]->addr, (uint64_t)PHYS_TO_VIRT(ioapics[i]->addr));
